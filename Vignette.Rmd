...
The humcap package has three functions for constructing maps. The first function is  unimap(), which takes a vector of states data, and plot a single map.
The scond function is multimap(), which takes a matrix of states data, and plots a frame of several small maps.
...
Example with a single vector is provided here:
```{r, fig.width= 7.2}
hc_map(testmod2$All[,1]) # Plot a single map
```

Example with a matrix of data is provided here:
```{r, fig.height= 6, fig.width= 7.2}
